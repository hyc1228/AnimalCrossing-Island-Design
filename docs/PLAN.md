...
2. ✅ **HID PNG 解码器（只读预览版）** `/import-hid`：完整复用 HID 的 LZ-String + 隐写格式，解出 v1/v2 JSON，按 `category/type` 聚合显示物品分布，下载完整 JSON。
   - 文件：`src/import/steganography.ts`、`src/import/hidDecoder.ts`、`src/pages/ImportHidPage.tsx`
   - ⏳ 待续：建一份 `HID.category_type → 我们的 itemKey` 映射表，才能"导入到画布"
3. **NH 数据接入 ItemPalette**：物品库从 60 件扩到 2075 件（需虚拟滚动 + 分类重组）
4. **PNG 导出 + URL 分享**：把当前岛屿 stage 截图 + 压缩 JSON 编进 URL hash
5. **平面图相似度匹配**：在灵感库里按"风格 + 物品集合"找相似 → 推荐已有灵感
6. **AI 文本驱动生成**：选风格 + 中文描述 → 规则引擎 + LLM hint
...
第二迭代第二批：
1. **HID category→itemKey 映射表**：在 `src/import/hidCatalog.ts` 维护 HID 物品分类到本工具 itemKey 的对照；让 ImportHidPage 多一个"导入到新岛屿"按钮。
2. **NH 数据接入 ItemPalette**：把 `NH_FURNITURE` (2075 件) 暴露在物品库里；要求虚拟滚动 + 分类树重组。
3. **画布 PNG 导出**：编辑器加"导出 PNG"按钮，调用 Konva `stage.toDataURL`，下载。
4. **URL 分享**：编辑器 JSON 走 lz-string `compressToEncodedURIComponent` → URL hash；HomePage 检测 hash 自动导入。
5. **平面图相似度匹配**：在 `/inspirations` 加"找相似"按钮，按 styleScores 余弦相似 + 物品集合 Jaccard 排序。
