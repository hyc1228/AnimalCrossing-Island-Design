...
- ✅ **统一鼠标指针**：把 animal-island-ui 在 Select 下拉菜单（首页时钟附近的 LanguageSwitcher）做的那个 AC 风格指针 `select-cursor.svg` 拷到 `public/cursor-ac.svg`，在 `src/index.css` 的 base 层定义两个 CSS 变量 `--ac-cursor` / `--ac-cursor-pointer`，让 `body` 用作默认、`a/button/select/role=button/checkbox/radio` 等用作 pointer 变体；输入框 / textarea / contenteditable 保留原生 I-beam，disabled 控件保留 not-allowed。`(hover: none) and (pointer: coarse)` 媒体查询里全部还原原生，避免触摸设备多吃带宽。`IslandCanvas` select 模式的 cursor 从 `'default'` 改成 `var(--ac-cursor)`，画布也同步使用 AC 指针；其他工具 (`pan/place/erase`) 的语义 cursor 保留不变。
- ✅ **AI 模型 catalog 刷新 + key 自动识别**（2026-05）：`settingsStore` 把 model 列表从 Gemini 2.0 / GPT-4o-mini / Claude 3.5 升级到当前最优——Gemini 3 Flash / 3.1 Pro / 3.1 Flash-Lite、GPT-5.4 mini / 5.4 / 5.4-nano / 5.5、Claude Haiku 4.5 / Sonnet 4.6 / Opus 4.7（保留 GPT-4o-mini & Gemini 2.5 Flash 作为 legacy 兜底）。默认模型同步更新为各家 cheap + vision 推荐档。新增 `detectProviderFromKey(rawKey)`，根据前缀（`sk-ant-` → Anthropic、`AIza` → Gemini、`sk-` → OpenAI）识别 provider；`SettingsDialog` 在 key 前缀和当前 provider 不匹配时显示"看起来是 X 的 key · 切换 →"提示 chip，不强制切换（避免误判用自定义 endpoint 的人）。`loadInitial` 加迁移逻辑：本地存的 model 若不在新 catalog 里自动落到当前默认，避免老用户开打就报 model not found。三语 i18n 同步更新定价说明 + `detectedProvider` / `detectedProviderSwitch`。
